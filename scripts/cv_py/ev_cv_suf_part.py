...
import metrics
from ev import read_res_pred_files_part
import math
...
def stats_cv(path=".",  format="trec", prefix="svm",  th=50, suf="",  verbose=False, ignore_noanswer=False,
  ...
  total_folds = len(os.listdir(path))
  for fold in sorted(os.listdir(path)):
    ...
    ir, svm = read_res_pred_files_part(res_fname, pred_fname, format, verbose,
                                  ignore_noanswer=ignore_noanswer,ignore_allanswer=ignore_allanswer,
                                       fold_num=fold_num, total_fold=total_folds)
    '''
    except:
  ...
  print "\nMRR\tMAP\tP@1d"
  print u"%5.2f \u00B1 %4.2f\t%5.2f \u00B1 %4.2f\t%5.2f \u00B1 %4.2f" % (avg_mrr_svm, std_mrr_svm, avg_map_svm*100,
                                                   std_map_svm*100, avg_rec1_svm, std_rec1_svm)
