...
def get_arg_parser():
  ...
  # pararellize_annotation
  parser.add_argument('--pararellize_annotation', help='launch train/test/dev data structures extraction and conversion '
                                                       'to svmlighttk format in background in parallel',
                      dest='pararellize_annotation', default=False,  action='store_true')
...
def trecv_commands(corpus_info, system_class, experiment_class, experiment_name, base_output_folder,model_suf,svm_params,corpus_name,add_java, read_feats_from_file,
                   str_time, run_eval=True, skip_training_and_eval=False):
    cb = CommandsBatch()
    ...
    logging.info("Experimental data will be written to %s" % expfolder)
    if experiment_class is not None:
        java_command_train = command_pattern % (MAXMEM,system_class, corpus_info.get_info(TRAIN_STR)[QUESTION_FILE],
                                                corpus_info.get_info(TRAIN_STR)[ANSWER_FILE], expfolder,
                                                corpus_info.get_info(TRAIN_STR)[CAS],
                                                corpus_info.get_info(TRAIN_STR)[CAND_TO_USE],
                                                corpus_info.get_info(TEST_STR)[CAND_TO_USE],
                                                experiment_class, add_java, expid)
...
def run_train_dev_test(corpus_info, system_class, experiment_class, experiment_name, base_output_folder,
                       ...
                       str_time, add_java_train="", add_java_dev="", add_java_test="",
                       ...
                       skip_training_and_eval=False, pararellize_annotation = True):

    #str_time = time.strftime("%d-%m-%Y");
    expid=corpus_name+"_"+experiment_name+"_"+str_time;
    expfolder=os.path.join(base_output_folder, expid)
    logging.info("Writing train/test/dev data to %s" %(expfolder))
    java_command_pattern = "java -XX:ParallelGCThreads=4 -Xmx%sG -Xss512m %s -questionsPath %s -answersPath %s " \
                                 "-outputDir %s -filePersistence %s -candidatesToKeep %s -expClassName %s -mode %s %s > logs/java_%s_%s.log 2>&1"
    if pararellize_annotation:
        java_command_pattern = "nohup %s &" % java_command_pattern

    cb = CommandsBatch()
    modes = [(TRAIN_STR, add_java_train), (DEV_STR, add_java_dev)] if only_development \
        else [(TRAIN_STR, add_java_train), (DEV_STR, add_java_dev), (TEST_STR, add_java_test)]
    if experiment_class is not None:
        for mode, add_java_params in modes:
            if corpus_info.get_info(mode) is not None:
                add_java_mode = add_java + " "+ add_java_params
                if read_feats_from_file:
                    add_java_mode = add_java_mode + " -featureCacheFileName %s" % \
                                     (corpus_info.get_info(mode)[FEATURES_VALUES])
                java_command = java_command_pattern % (MAXMEM, system_class, corpus_info.get_info(mode)[QUESTION_FILE],
                                                           corpus_info.get_info(mode)[ANSWER_FILE],
                                                           expfolder,
                                                           corpus_info.get_info(mode)[CAS],
                                                           corpus_info.get_info(mode)[CAND_TO_USE],
                                                           experiment_class,
                                                           mode,
                                                           add_java_mode,
                                                           mode,
                                                           expid)
                cb.add_java(java_command)
    ...
    '''
    =====GENERATE SVM PREDICTION COMMANDS=====
    ...
    If there is a development set, generate the commands for prediction and evaluating on it
    '''
    eval_modes = [DEV_STR] if only_development else [DEV_STR, TEST_STR]
    for eval_mode in eval_modes:
        if corpus_info.get_info(eval_mode)!=None:
            eval_file = "svm.%s" % (eval_mode)
            pred_file_name = "svm_%s.%s.pred" % (model_suf, eval_mode)
            svm_command_pred = svm_command_test_pattern % (svm_light_classify_location, os.path.join(expfolder, eval_file),
                                                          os.path.join(expfolder, model_file_name),
                                                          os.path.join(expfolder, pred_file_name), eval_mode, expid, model_suf)
            cb.add_test(svm_command_pred)

            if run_eval:
                relfile = os.path.join(expfolder,"svm.%s.relevancy" %(eval_mode))
  ...
  for corpus_name in args.exps_to_run:
      ...
      if corpus_cross_validation or args.cross_validation==True:
        ...
        cb = trecv_commands(ci,args.cv_system_class,args.exp_no_ir, args.exp_code, base_exp_folder, args.suf, args.params,corpus_name,add_java.strip(),
                            ...
                            run_eval=args.run_eval, skip_training_and_eval=args.only_data_generation)
      else:
        cb = run_train_dev_test(ci, args.system_class, args.exp_no_ir, args.exp_code, base_exp_folder, args.suf, args.params, corpus_name,
                                ...
                                only_development=args.only_development,
                                ...
                                pararellize_annotation=args.pararellize_annotation)
