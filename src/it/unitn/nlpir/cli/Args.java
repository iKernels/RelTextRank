...
public class Args {
	...
	private static List<Field> getAllFieldsRec(List<Field> fields, Class<?> type) {
	    ...
	    if (type.getSuperclass() != null) {
	        fields = getAllFieldsRec(fields, type.getSuperclass());
	    }
	...
	private static List<Field> getAllFields(Class<?> type) {
		return getAllFieldsRec(new LinkedList<Field>(), type);
	}
    ...
    public static void usage(PrintStream errStream, Object target) {
        Class<?> clazz;
        Set<String> processedFieldsAndProperties = new HashSet<String>();
        if (target instanceof Class) {
            clazz = (Class) target;
        } else {
            clazz = target.getClass();
        ...
        for (Field field : getAllFields(clazz)) {
        	if (!processedFieldsAndProperties.contains(field.getName())){
        		fieldUsage(errStream, target, field);
        		processedFieldsAndProperties.add(field.getName());
        	}
    ...
    private static void fieldUsage(PrintStream errStream, Object target, Field field) {
        ...
        if (argument != null) {
            ...
            Boolean mandatory = argument.required();
            makeAccessible(field);
            try {
                Object defaultValue = field.get(target);
                Class<?> type = field.getType();
                propertyUsage(errStream, prefix, name, alias, type, delimiter, description, defaultValue, mandatory);
    ...
    private static void propertyUsage(PrintStream errStream, String prefix, String name, String alias, Class<?> type, String delimiter, String description, Object defaultValue, Boolean mandatory) {
